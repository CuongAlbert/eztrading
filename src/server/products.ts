...
export const getRecommendedProductsFromCategories = async (
  categories: string[],
) => {
  const categoryQuery = categories
    .map((category) => `"${category}" in categories`)
    .join(" || ");

  const groq = `*[_type == "products" && (${categoryQuery})] | order(_createdAt desc) [0...8] {
        _id,
        name,
        unit,
        "providers": providers->company,
