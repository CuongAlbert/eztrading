...
export const GalleryEdit: React.FC<GalleryEditProps> = ({
  ...
  const updateProviderGallery = async () => {
    if (!selectedImages || selectedImages.length === 0) return;
    try {
      const assetPromises = selectedImages.map((image) =>
  ...
  const onSaveGallery = async () => {
    if (!selectedImages || selectedImages.length === 0) return;
    setIsUpdating(true);
    const data = await updateProviderGallery();
  ...
  return (
    <div className="flex flex-col gap-4">
          ...
          {previewUrls &&
            ...
            previewUrls.map((url, index) => (
              <div className="w-full aspect-square relative" key={index}>
                ...
                <div className="absolute top-2 right-2 bg-slate-1 rounded-md p-1 ">
      ...
      <div className="flex gap-4 flex-1">
        ...
        <button
          className={`btn ${
            !previewUrls || previewUrls.length === 0 || isUpdating
              ? "btn-disabled"
              : "btn-primary"
          } w-full flex-1`}
          onClick={
        ...
        >
          {isUpdating ? (
            <span className="loading loading-infinity text-blue-12" />
          ) : (
            <p>Save</p>
          )}
        </button>
